...
function static_class(CID){
  ...
  const fetchDataStatis = async() => {
     ...
     var month = []
     var MM = new Date(data_statis[0].Time).getMonth()+1;
     month.push(MM);
     for (let i = 0; i < data_statis.length; i++) {
         if (MM !== new Date(data_statis[i].Time).getMonth()+1){
             month.push(new Date(data_statis[i].Time).getMonth()+1)
             MM = new Date(data_statis[i].Time).getMonth()+1;
         }
     }
     console.log(month);
     var dataByMonth = [];
     for (let i = 0; i < month.length; i++){
         ...
         for(let j = 0; j < data_statis.length; j++){
             if(month[i] === new Date(data_statis[j].Time).getMonth()+1){
